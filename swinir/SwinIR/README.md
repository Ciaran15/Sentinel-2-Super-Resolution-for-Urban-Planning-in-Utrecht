...
## Adaptation for Sentinel-2 Super-Resolution (Urban Planning in Utrecht)

This folder contains an adapted implementation of SwinIR specifically configured for **Sentinel-2 to aerial imagery super-resolution** for urban planning applications in Utrecht. The model is used to enhance low-resolution Sentinel-2 satellite imagery to match the resolution and detail level of high-resolution aerial reference data (PDOK).
...
- `scikit-image` - Image analysis and metrics
- `Pillow` - Image manipulation
- `basicsr==1.4.2` - Image restoration framework
- `cog` - Container framework for model deployment

### Dataset Structure

For fine-tuning on paired Sentinel-2 to aerial imagery data, organize your dataset as follows:

```
dataset/
├── train/
│   ├── inputs/          # Low-resolution Sentinel-2 images
│   └── ground_truth/    # High-resolution aerial reference images (PDOK)
└── val/
    ├── inputs/          # Validation LR Sentinel-2 images
    └── ground_truth/    # Validation HR aerial reference images
```

**File Matching:**
- Images are matched by **filename stem** (e.g., `tile_001.tif` in inputs matches `tile_001.tif` in ground_truth)
- Supported formats: `.tif`, `.tiff`, `.png`, `.jpg`
- Images should be properly georeferenced and co-registered

### Model Weights

Pre-trained SwinIR model weights are available in the `model_zoo/` directory:

| Model | Purpose | File |
|-------|---------|------|
| SwinIR-M (Medium) | Real-world SR baseline | `003_realSR_BSRGAN_DFO_s64w8_SwinIR-M_x4_GAN.pth` |
| SwinIR-M Alternative | Real-world SR variant | `001_classicalSR_DF2K_s64w8_SwinIR-M_x4.pth` |
| SwinIR-L (Large) | Large model for better quality | `003_realSR_BSRGAN_DFOWMFC_s64w8_SwinIR-L_x4_GAN.pth` |

These weights are automatically loaded during inference. For fine-tuning on Sentinel-2 data, these pre-trained models serve as initialization.

### Usage

Detailed instructions for running inference and fine-tuning are provided in the notebook files:

- **Inference:** See `SwinIR_Inference.ipynb` in the `Notebooks/` folder
- **Fine-tuning:** See `Swinir_Optuna.ipynb` for hyperparameter optimization and fine-tuning workflows
- **Evaluation:** See`SwinIR_Evaluation.ipynb` for validation metrics and comparisons

The main Python scripts in this directory (`main_test_swinir.py`, `predict.py`, `train_swinir_finetune.py`, `evaluate_sr_against_pdok.py`) are also documented with detailed docstrings.
