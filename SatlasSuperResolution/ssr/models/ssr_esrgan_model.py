...
class SSRESRGANModel(SRGANModel):
    ...
    def _freeze_layers(self, model, freeze_patterns):
        """
        Freeze layers matching the given patterns.
        ...
        logger.info(f'Transfer Learning: Froze {frozen_count} parameters, {trainable_count} trainable')
    ...
    def init_training_settings(self):
        ...
        freeze_layers = self.opt.get('freeze_layers', [])
        if freeze_layers:
            logger = get_root_logger()
            logger.info(f'Freezing layers for transfer learning: {freeze_layers}')
            self._freeze_layers(self.net_g, freeze_layers)
            if self.ema_decay > 0:
                self._freeze_layers(self.net_g_ema, freeze_layers)
    ...
    def feed_data(self, data):
        ...
        self.old_hr = None
        if 'old_hr' in data:
            self.old_hr = data['old_hr'].to(self.device).float()
            if self.old_hr.max() > 1.0:
                self.old_hr /= 255.0
