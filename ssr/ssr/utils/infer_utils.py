...
def format_s2naip_data(s2_data, n_s2_images, device):
    # Accept either a stacked Sentinel-2 patch array or a single 64x64 image.
    if s2_data.ndim != 3 or s2_data.shape[2] != 3:
        raise ValueError("Input image must be HxWx3.")

    h, w, _ = s2_data.shape
    if h == 64 and w == 64 and n_s2_images == 1:
        # Single 64x64 image input.
        s2_chunks = np.expand_dims(s2_data, axis=0)
        s2_image = s2_data
    else:
        ...
        try:
            ...
            raise ValueError(
                "Input image must be either a 64x64 single image with n_lr_images=1 "
                "or a stacked 32x32 image of shape (T*32, 32, 3)."
            )
    ...
    # Return input of shape [batch, n_s2_images * channels, H, W].
